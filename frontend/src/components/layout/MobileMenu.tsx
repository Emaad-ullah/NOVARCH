...
import { X, ChevronDown, ArrowRight, Sun, Moon } from 'lucide-react';
      ...
      >
        ...
        <div className="flex items-center justify-between border-b border-border px-5 py-4">
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={toggleTheme}
              className="rounded-sm p-1.5 text-text-muted hover:bg-surface-2 hover:text-text transition-colors cursor-pointer"
              aria-label="Toggle theme"
            >
              {theme === 'dark' ? <Sun className="h-5 w-5 text-yellow-400" /> : <Moon className="h-5 w-5 text-[#0D1B2A]" />}
            </button>
            <button
              onClick={handleClose}
